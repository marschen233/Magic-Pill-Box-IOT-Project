...
int main(void) {
	...
	FlagA = false;
	res = f_open(&file_object, (char const*)testA_file_name, FA_READ);	//read if flagA.txt is exist
	f_unlink(testA_file_name);	//after see the FlagA or FlagB, we can delete the Flag_txt file
	f_close(&file_object);
	if(res == FR_OK)
	{
		FlagA = true;		//if it find the testA_file_name, means that its going to load the A file
	}
	else
	{
		res = f_open(&file_object, (char const*)testB_file_name, FA_READ);	//when read flagB.txt, it will return FR_OK
		f_unlink(testB_file_name);	//after see the FlagA or FlagB, we can delete the Flag_txt file
		f_close(&file_object);
		if(res != FR_OK)		//if I did not find fileB, ill just default as flag A
		{
			FlagA = true;
		}
	}
	
	//after we select which file to load, we can start to delete the MCU application code
	bool DeleteFlg = DeleteApplication();
	// based on the flag.txt file we found, load the correspond bin file to the application code
	if(FlagA == true)
	{
		WriteApplication(testA_bin_file);
		
	}
	else{
		WriteApplication(testB_bin_file);
	}
